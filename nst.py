...
class StyleTransfer(nn.Module):
    def __init__(self):
        ...
        self.cnn = models.vgg19(pretrained=True).features.to(CFG.device).eval()

    def get_style_model_and_losses(self, style_img, content_img):
            cnn = copy.deepcopy(self.cnn)
            # normalization module
            normalization = Normalization(CFG.normalization_mean, CFG.normalization_std).to(CFG.device)
...
if __name__ == "__main__":
    style_img = image_loader(dir+"/winter.png")# as well as here
    content_img = image_loader(dir+"/summer.jpeg")#измените путь на тот который у вас.
